import React from 'react';
import { AlertProps } from './AlertProps';
import { RectButton } from '../Button';

const Alert: React.FC<AlertProps> = ({
  children,
  variant = 'default',
  dismissible = false,
  onDismiss,
  actionText,
  onAction,
  linkText,
  linkHref,
  playground = false,
  rtl = false,
  className = '',
  ...props
}) => {
  const getVariantClasses = (): string => {
    switch (variant) {
      case 'info':
        return 'bg-blue-900 ';
      case 'success':
        return 'bg-green-600 ';
      case 'warning':
        return 'bg-orange-700  text-yellow-100';
      case 'critical':
        return 'bg-red-700 ';
      default:
        return 'bg-primaryA';
    }
  };

  const getIconByVariant = () => {
    switch (variant) {
      case 'info':
        return (
          <svg className="w-5 h-5 text-blue-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
          </svg>
        );
      case 'success':
        return (
          <svg className="w-5 h-5 text-green-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
          </svg>
        );
      case 'warning':
        return (
          <svg className="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
          </svg>
        );
      case 'critical':
        return (
          <svg className="w-5 h-5 text-red-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
          </svg>
        );
      default:
        return null;
    }
  };

  return (
    <div
      className={` flex ${rtl ? 'flex-row items-end text-right' : 'flex-row text-left '} items-center p-4 mb-4 text-white ${getVariantClasses()} ${className}`}
      dir={rtl ? 'rtl' : 'ltr'}
      role="alert"
      {...props}
    >
      {getIconByVariant() && (
        <div className={`flex-shrink-0 ${rtl ? 'ml-3' : 'mr-3'}`}>
          {getIconByVariant()}
        </div>
      )}
      
      <div className="flex-1">
        <div className="text-sm font-medium flex items-center">
          {children}
          
          {linkText && linkHref && (
            <a 
              href={linkHref}
              className={`${rtl ? 'mr-2' : 'ml-2'} underline font-medium text-blue-400 hover:text-blue-300`}
            >
              {linkText}
            </a>
          )}
           {actionText && onAction && (
          <div className={`pl-4 ml-auto ${rtl ? 'text-left' : 'text-right'} `}>
            <RectButton label={actionText} variant='secondary' size='small' style={{border:'none'}} onClick={onAction}/>
          </div>
        )}
        </div>

       
      </div>

      {dismissible && onDismiss && (
        <button
          type="button"
          className={`${rtl ? 'mr-auto ml-2' : 'ml-auto mr-2'} bg-transparent text-white hover:text-gray-200 rounded-lg p-1.5 inline-flex items-center justify-center h-8 w-8`}
          onClick={onDismiss}
          aria-label="Close"
        >
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
          </svg>
        </button>
      )}
    </div>
  );
};

export default Alert;